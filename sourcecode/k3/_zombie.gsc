...
main()
{
	...
	level.zommaxhealth = int(level.zombiehealth);
...
precache()
{
	...
	level.zombiehealth = createdvar("btd_zombiehealth",150,100,10000,"int");
	level.zomwaves = createdvar("btd_zombiewaves",15,5,50,"int");
...
dmg(health)
{
	...
	{
		if(level.killsprees)
			attacker thread btd_messages\_killspree::check_for_rampage(self);

		if(level.multikill)
			attacker thread btd_messages\_multikill::multikill();
...
//		attacker iPrintlnBold("^7You killed a ^1Zombie!");
		...
		attacker thread maps\mp\gametypes\_rank::giveRankXP("kill", 20);
		attacker.score += 20;
	...
	if(!self.wasbugged)
	{
		for(i=0;i<self.attackers.size;i++)
		{
			...
			self.attackers[i] thread maps\mp\gametypes\_rank::giveRankXP("assist", 10);
			self.attackers[i].score += 10;
...
monitorplayers()
{
	...
	while(1)
	{
		...
		{
			wait 2;
			thread maps\mp\gametypes\_globallogic::endGame("axis", "^1The zombies have eaten all of us!");
		}
...
{
	if(level.killsprees)
		self thread btd_messages\_killspree::check_for_endoframpage(eInflictor,attacker);
