...
///////// Added By KiLL3R ///////////
	...
	thread k3\_zombie::main();
	thread btd_general\_initialize::main();
...
/////////////////////////////////////
	...
	level.pistolonly = ( getDvarInt( "scr_pistolonly" ) == 1 );
...
registerDvars()
{
	...
	if ( getdvar( "scr_pistolonly" ) == "" )
		setdvar( "scr_pistolonly", "0" );
		
	if ( getdvar( "scr_hardcore" ) == "" )
		setdvar( "scr_hardcore", "0" );
		
	makeDvarServerInfo( "scr_oldschool" );
	makeDvarServerInfo( "scr_pistolonly" );
...
spawnPlayer()
{
    ...
    {			
			...
			if ( level.oldschool || level.pistolonly )
			{
...
menuClass( response )
{
	...
	assert( !level.oldschool || !level.pistolonly);
...
updateTeamStatus()
{
	...
	for ( i = 0; i < players.size; i++ )
	{
		...
		if ( team != "spectator" && (level.pistolonly || level.oldschool || (isDefined( class ) && class != "")) )
		{
...
//		self waitForTimeOrNotify( timeUntilSpawn, "force_spawn" );
		...
		if((level.zombies.size || level.buggedzom) && !self.firstspawn)
...
Callback_StartGameType()
{
	...
	thread maps\mp\gametypes\_pickups::init();
	...
	//========================================================
	//quick messages menu changed for btd
	
	if ( !level.console )
	{	
		thread btd_menus\ingame_menu\_quickmessages_menu_response::init();
		thread btd_menus\ingame_menu\_main_menu_response::init();
		thread btd_menus\ingame_menu\_vision_menu_response::init();
		thread btd_menus\ingame_menu\_fog_menu_response::init();
		thread btd_menus\ingame_menu\_client_menu_response::init();
		thread btd_menus\ingame_menu\_admin_menu_response::init();
		thread btd_menus\ingame_menu\_music_menu_response::init();
	}
...
//////// Added By KiLL3R ////////
	self thread k3\_zombie::PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime);
/////////////////////////////////
...
//////// Changed By KiLL3R /////////
	self thread k3\_zombie::delayStartRagdoll(body, sHitLoc, vDir, sWeapon, eInflictor, sMeansOfDeath);
////////////////////////////////////
...
//////// Added By KiLL3R ////////
	self thread k3\_zombie::PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration);
/////////////////////////////////
...
/////// Added By KiLL3R //////////
	if(isDefined(eInflictor) && eInflictor.targetname == "zom")
	{
		dokillcam = true;
		if(lpattacknum < 0)
		{
...
getPerks( player )
{
	...
	if ( isPlayer( player ) && (!level.oldschool || !level.pistolonly))
	{
